// No provider tokens or session cookies are read by this module.
...
const errorMessages = {
  ...
  CSRF_REJECTED:'로그인 상태를 새로고침한 뒤 다시 시도해 주세요.', ORIGIN_REJECTED:'게임 서버와 같은 주소에서 접속해 주세요.',
};
export function initLeague(game) {
  let config=null, me=null, csrf='', rooms=[], roomId='', period='daily', rankingVersion=0, pending=false, confirmation=null, sdkPromise=null;
  let invite='';
  ...
  function showError(error){message(errorMessages[error.code] || '연결이 원활하지 않아요. 기록은 이 브라우저에 남아 있으니 잠시 후 다시 시도해 주세요.',true);}
  ...
  function renderAccount(){
    ...
    $('#league-nickname').textContent=me?.nickname||'';$('#league-join').disabled=!me;$('#league-join').textContent=me?'이 방에 참가하기':'먼저 카카오 로그인';
    ...
    $('#friends-card-label').textContent=me?me.nickname+' · 내 친구방 '+rooms.length+'개 ↗':'카카오로 연결하고 시작하기 ↗';
  }
  async function refreshMe(){const data=await api('/me');me=data.user;csrf=data.csrfToken||'';rooms=data.rooms||[];if(invite&&rooms.some(r=>r.id===invite)){roomId=invite;clearInvite();}renderAccount();}
  async function showInvite(){if(!invite)return;const data=await api('/invites/'+invite);$('#league-invite').hidden=false;$('#league-invite-name').textContent=data.name;$('#league-invite-description').textContent=data.memberCount+'명이 기다리는 방이에요. 참가하면 게임 닉네임과 등록한 기록이 방 안에 공개돼요.';}
  async function refreshRanking(){
    ...
    $('#league-kakao-share').hidden=!config?.kakaoJavascriptKey;$('#league-native-share').hidden=!navigator.share;renderAttempt();
  }
  ...
  function loadKakao(){
    ...
    sdkPromise=new Promise((resolve,reject)=>{const script=document.createElement('script');script.src='https://t1.kakaocdn.net/kakao_js_sdk/2.8.3/kakao.min.js';script.crossOrigin='anonymous';script.onload=()=>{try{if(!window.Kakao.isInitialized())window.Kakao.init(config.kakaoJavascriptKey);resolve(window.Kakao);}catch(error){reject(error);}};script.onerror=reject;document.head.append(script);});
  ...
  $('#league-kakao-share').addEventListener('click',()=>action($('#league-kakao-share'),async()=>{const Kakao=await loadKakao();const url=inviteURL();Kakao.Share.sendDefault({objectType:'text',text:$('#league-room-title').textContent+'에서 2048 한 판! 같은 일일 퍼즐로 우리끼리 기록 경쟁해요.',link:{mobileWebUrl:url,webUrl:url},buttonTitle:'친구방 참가하기'});message('카카오톡에서 보낼 친구나 채팅방을 선택해 주세요.');}));
  ...
  $('#league-logout').addEventListener('click',()=>action($('#league-logout'),async()=>{await api('/logout',{method:'POST'});me=null;csrf='';rooms=[];roomId='';renderAccount();message('로그아웃했어요. 혼자 플레이는 계속할 수 있어요.');}));
  ...
  $('#league-delete-account').addEventListener('click',()=>ask('모든 친구방 참가 정보, 게임 계정과 서버 기록을 영구 삭제할까요? 다른 친구의 기록은 남겨요. 카카오의 서비스 연결은 카카오계정 설정에서 별도로 해제할 수 있어요.',()=>action($('#league-delete-account'),async()=>{await api('/me',{method:'DELETE'});me=null;csrf='';rooms=[];roomId='';renderAccount();message('서버의 내 게임 데이터를 삭제했어요. 이 브라우저의 혼자 플레이 기록은 남아 있어요.');})));
  ...
  (async()=>{try{config=await api('/config');if(!config.enabled)return;$('#storage-note').textContent='혼자 플레이는 기기에, 친구 대결은 서버에 저장돼요.';$('#friends-card').hidden=false;$('#mode-league').hidden=false;$('.mode-tabs').classList.add('has-league');if(params.has('league')||invite)await open();}catch{if(params.has('league')||invite)await open();}})();
  return {open};
}
