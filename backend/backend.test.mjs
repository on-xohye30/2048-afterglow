...
function fixture() {
  ...
  const worker=createWorker({now:()=>time,fetchImpl:async (url,options)=>{
    if(url==='https://kauth.kakao.com/oauth/token'){
      const code=options.body.get('code');if(code==='denied')return new Response('{}',{status:400});
      return Response.json({access_token:'mock-'+code});
    }
    if(url==='https://kapi.kakao.com/v2/user/me'){
      const id=options.headers.Authorization.replace('Bearer mock-','');
      return Response.json({id:Number(id),kakao_account:{profile:{nickname:'테스터'+id}}});
    }
  ...
  function client(){
    ...
    return {req,login:async id=>{const start=await req('/api/auth/kakao');assert.equal(start.status,302);const state=new URL(start.response.headers.get('Location')).searchParams.get('state');const callback=await req('/api/auth/kakao/callback?state='+state+'&code='+id);assert.equal(callback.status,302);assert.ok(callback.response.headers.getSetCookie().some(cookie=>cookie.startsWith('__Host-afterglow_session=')&&cookie.includes('HttpOnly')&&cookie.includes('Secure')&&cookie.includes('SameSite=Lax')));assert.equal(callback.response.headers.get('Location'),ORIGIN+'/?league=1');const me=await req('/api/me');csrf=me.value.csrfToken;assert.ok(csrf);return me.value.user;}};
  }
...
test('config does not claim authentication when secrets or origin are missing',async()=>{const f=fixture();const r=await f.worker.fetch(new Request(ORIGIN+'/api/config'),{DB:f.db});assert.equal((await r.json()).authConfigured,false);const staticR=await f.worker.fetch(new Request(ORIGIN+'/'),f.env);assert.equal(staticR.headers.get('X-Content-Type-Options'),'nosniff');});
test('OAuth state is mandatory, single-use and bounded',async()=>{const f=fixture(),c=f.client();const bad=await c.req('/api/auth/kakao/callback?state=bad&code=101');assert.match(bad.response.headers.get('Location'),/authError/);assert.equal((await c.req('/api/me')).value.user,null);const start=await c.req('/api/auth/kakao');const state=new URL(start.response.headers.get('Location')).searchParams.get('state');await c.req('/api/auth/kakao/callback?state='+state+'&code=101');const again=await c.req('/api/auth/kakao/callback?state='+state+'&code=101');assert.match(again.response.headers.get('Location'),/authError/);});
test('login stores only hashed session and sets secure cookies, no provider token disclosure',async()=>{const f=fixture(),c=f.client();await c.login(101);const row=f.db.db.prepare('SELECT token_hash FROM sessions').get();assert.match(row.token_hash,/^[a-f0-9]{64}$/);const me=await c.req('/api/me');assert.deepEqual(Object.keys(me.value.user).sort(),['id','nickname']);assert.equal(me.response.headers.get('Cache-Control'),'no-store');assert.equal(me.response.headers.get('Access-Control-Allow-Origin'),null);});
