...
- 공개 기록 링크는 `play=1`로 새 공유판을 시작합니다. 보드·계정·친구방 값은 URL에 넣지 않습니다.
- 일반 저장과 별도인 공유판은 최근 8개를 보관합니다. 새 탐색은 새 판, 같은 탭의 새로고침/뒤로 가기는 해당 판 복원입니다. 기존 일반 저장은 모드 선택의 '이전 판 이어하기'로 복귀합니다.
- 자동 테스트 149개: 공유 URL 정리, 날짜 검증, 기존 저장 보호, 탭 분리, 새로고침, 자정 전환, 친구 대결 전환 등을 포함합니다.
- 격리된 미리보기에서 새 공유판 0점/0수, 플레이 후 새로고침 유지, 설정·방향 버튼 숨김·도움말·공유 메뉴를 브라우저로 확인했습니다.
- 320/390px 및 960×600 배치는 정적 CSS 검토를 통과했습니다. 모바일 iframe 검증은 기존 프레임 차단 보안 정책에 막혀 중단했습니다. 보안 헤더를 변경하지 않았으며 모바일 실기기·실제 카카오톡 전달 검증으로 주장하지 않습니다.
