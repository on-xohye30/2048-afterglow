...
function renderStats(){
  ...
  else if(state.over){$('#overlay-kicker').textContent='NICELY PLAYED';$('#overlay-title').textContent='이번 판도 멋졌어요.';$('#overlay-copy').textContent=fmt(state.score)+'점 · 가장 큰 타일 '+fmt(top)+' · '+fmt(state.moves)+'수';$('#overlay-primary').textContent=mode==='league'?'기록 등록하고 순위 보기':mode==='daily'?'같은 퍼즐 다시 도전':'다시 도전';$('#overlay-secondary').textContent='한 수 되돌리기';$('#overlay-secondary').hidden=!undoAvailable();}
}
...
async function move(direction){
  ...
  if(mode==='league'&&(state.run.day!==dateInSeoul()||state.trace.length>=10000)){toast(state.trace.length>=10000?'이동 한도에 도달했어요. 친구방에서 기록을 등록해 주세요.':'날짜가 바뀌었어요. 친구방에서 오늘의 대결을 시작해 주세요.');return;}
  ...
  if(mode==='league')state.trace.push(direction);
  ...
  renderBoard(spawned.index,result.merges);renderStats();if(mode==='league')document.dispatchEvent(new CustomEvent('afterglow:league-progress'));playTone(result.merges.length>0);
...
initLeague({
  ...
  markSubmitted:id=>{const stored=storedLeagueState();if(stored?.run?.id!==id)return;stored.submitted=true;if(mode==='league'){state=stored;renderAll();persist();}else{try{localStorage.setItem(GAME_PREFIX+'league',JSON.stringify(stored));}catch{}}},
});
